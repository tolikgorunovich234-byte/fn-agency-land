  ...
  return (
    <div className="min-h-screen w-full bg-white text-black">
          ...
          <div className="mt-8 md:mt-10 grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-3">
            {[
              ...
              <motion.div
                ...
                className="group relative overflow-hidden rounded-2xl border bg-white p-4 sm:p-5 shadow-sm"
              >
                ...
                <div className="text-[11px] sm:text-xs uppercase opacity-60">{c.k}</div>
                <div className="mt-1 text-base sm:text-lg font-bold uppercase">{c.t}</div>
          ...
          <div className="mt-8 md:mt-10 grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-4">
            {[
              ...
              <div key={i} className="rounded-2xl border p-4 sm:p-5">
                <div className="text-[11px] sm:text-xs opacity-60">{s.n}</div>
                <div className="mt-1 text-base sm:text-lg font-bold uppercase">{s.t}</div>
                <div className="mt-2 text-sm opacity-80">{s.d}</div>
              </div>
            ))}
          </div>

          <div className="mt-8 md:mt-10 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 md:gap-4 rounded-2xl border p-4 sm:p-5">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="w-full md:w-auto rounded-full bg-white px-5 py-3 text-center text-sm font-semibold uppercase tracking-wider text-black hover:opacity-90"
            >
        ...
        <div className="mx-auto max-w-6xl px-3 sm:px-4 py-12 md:py-20">
          ...
          <div className="mt-8 md:mt-10 grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-3">
            {[
              ...
              <div key={i} className="rounded-2xl border p-4 sm:p-5">
                <div className="text-base sm:text-lg font-bold uppercase">{c.t}</div>
                <div className="mt-2 text-sm opacity-80">{c.d}</div>
              </div>
            ))}
          </div>
      ...
      <section id="lead" className="border-y bg-neutral-950 text-white">
        <div className="mx-auto max-w-6xl px-3 sm:px-4 py-12 md:py-20">
          <div className="flex flex-col md:flex-row items-start gap-5 md:items-center md:justify-between">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border px-3 py-1 text-[10px] sm:text-xs uppercase tracking-wider">
                Готов к обряду посвящения?
              </div>
              <h3 className="mt-3 sm:mt-4 [text-wrap:balance] text-2xl sm:text-3xl md:text-5xl font-extrabold uppercase tracking-tight">
                Начни запуск прямо сейчас
              </h3>
              <p className="mt-2 sm:mt-3 max-w-xl text-sm opacity-80">
                Нажми на кнопку, пройди мини-квиз или сразу напиши нам в Telegram.
              </p>
            </div>
            <div className="w-full md:w-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              <button
                onClick={startQuiz}
                className="w-full sm:w-auto rounded-full bg-white px-6 py-3 text-sm font-semibold uppercase tracking-wider text-black hover:opacity-90"
              >
                Пройти мини-квиз
              </button>
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-full sm:w-auto text-center rounded-full border px-6 py-3 text-sm font-semibold uppercase tracking-wider hover:bg-white hover:text-black"
              >
                Написать в Telegram
              </a>
            </div>
          </div>
          <div className="mt-4 sm:mt-6 text-[11px] sm:text-xs uppercase tracking-wider opacity-70">
      ...
      <footer className="bg-white">
        <div className="mx-auto max-w-6xl px-3 sm:px-4 py-10 sm:py-12 text-[11px] sm:text-xs opacity-70">
          © FN Agency • Связь:{" "}
          <a href="http://t.me/FMARKETING_studio" target="_blank" rel="noreferrer" className="underline">
            @FMARKETING_studio
          </a>
        </div>
      </footer>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button onClick={closeQuiz} className="rounded-full border px-2 py-1 text-[11px] sm:text-xs uppercase">Закрыть</button>
              </div>
              <div className="mt-2 sm:mt-3 text-xl sm:text-2xl font-extrabold uppercase">Мини-квиз</div>

              {step < steps.length ? (
                <div className="mt-4 sm:mt-6">
                  <div className="text-sm opacity-80">{steps[step].q}</div>
                  <div className="mt-3 sm:mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3">
                  ...
                  <div className="mt-3 sm:mt-4 text-[11px] sm:text-xs uppercase tracking-wider opacity-60">Шаг {step + 1} из {steps.length}</div>
                </div>
              ) : (
                <div className="mt-4 sm:mt-6">
                  <div className="text-sm opacity-80">Готово. Твои ответы:</div>
                  ...
                  <div className="mt-2 sm:mt-3 text-[10px] sm:text-[11px] uppercase tracking-wider opacity-60">
          ...
          >
            ...
            >
              ...
              <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                <button
                  onClick={() => { setCowardOpen(false); startQuiz(); }}
